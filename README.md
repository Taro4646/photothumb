...
- **デザイン提案（ルール）** — 出力比率のまま写真を 48px 幅にサンプリングし、領域ごとの明るさと情報量を計測。
  79 書体それぞれに「太さ感・ストロークの強弱・有機性・モダン度・小サイズ可読」の 5 指標を持たせ、
  写真の性格から求められる書体像を数値で出して全書体を採点する。構図の型は 7 つ用意し、
  写真ごとに適合度を採点して上位 3 つだけを提示する
- **デザイン提案（AI・任意）** — 自分の Anthropic API キーを入れると、写真と計測値を送って
  書体・配置・コピー案を返させられる。AI が選べるのはアプリが実際に持つ書体 ID と 15 個の配置アンカーだけで、
  座標・級数・コントラスト・沈め量はローカルで計算し直すため数値の幻覚が混ざらない。
  キーは端末内にのみ保存され、押したときだけ通信するのでオフライン動作は保たれる
